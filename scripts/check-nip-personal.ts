...
async function main() {
  ...
  try {
    ...
    const rol = await Rol.create({ nombre: "Operador prueba", ambito: "matriz", permisos: ["pos.vender"], esSupervisor: false });
    ...
    assert.equal((await PATCH(request("PATCH", { nipOperacion: null }), { params: Promise.resolve({ id }) })).status, 400);
